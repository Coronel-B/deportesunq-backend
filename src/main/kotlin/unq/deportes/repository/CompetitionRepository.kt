...
interface CompetitionRepository : CrudRepository<Competition, Long> {
    @Query("select new unq.deportes.model.CompetitionPositions(cp.position, cp.points, t) from Team t, CompetitionDetailItem cp where cp.competitionId.id = :id and t.id = cp.team order by cp.position")
